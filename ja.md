...
| | 必要なもの | 費用 |
|---|---|---|
| **かんたん構成** | パソコン、OpenAI の API キー | AI の利用料（従量制・少額） |
...
4つ質問されるので、答えてください。分からなければ Enter を押せば既定値になります。

```
1) AI をどこで動かしますか？     → 1（クラウド）
2) API キーを入力してください    → 貼り付ける
3) 組織を識別する名前            → 会社名など（英小文字）
4) データベース連携を使いますか？ → N
```

**API キーの取り方**
https://platform.openai.com/api-keys でアカウントを作り、キーを発行します。
`sk-` で始まる長い文字列です。他人に見せないでください。
