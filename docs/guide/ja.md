...
**すべて無料です。** 機能制限版でも試用版でもありません。
テンプレートもプロンプトも同じ条件で使えます。
（AI サービスの利用料は、選んだ提供元に直接支払うものです）
...
**AI の提供元は5つから選べます。** 迷ったら OpenAI にしてください。
...
| Claude | 文章の質が高い。ただし鍵が2つ要る（埋め込みAPIが無いため） |
...
trigger: "event:teams:meeting_ended"   # 起動のきっかけ（自動では動かない）
...
`event:` は「何がきっかけか」を書く欄です。会議が終わっても自動では動きません。
`aipmo run` かスマホの画面から、会議の情報を渡して実行します。
決まった時刻に動かすなら `trigger: "schedule:..."` と `aipmo schedule` です。
...
**公開は自動では行われません。** テンプレートから公開用の保管場所へ
書き込むことはできません。候補を出すところまでです。
